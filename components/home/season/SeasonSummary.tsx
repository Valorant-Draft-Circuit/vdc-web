...
import { RecapStat, RecapViewKey } from "@/lib/common/matchNight/types";
...
export default function SeasonSummary({ summary, mapUuidsByName, season }: Props) {
  ...
  const viewByKey = new Map<RecapViewKey, SeasonView>([
    ["OVERALL", summary.overall],
    ...summary.tierViews.map((view) => [view.key, view] as const),
  ]);
  const activeView = viewByKey.get(activeViewKey) ?? summary.overall;
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <div className="hidden sm:flex flex-wrap gap-1.5 sm:ml-auto">
              {pillKeys.map((key) => {
                ...
                const isDisabled = key !== "OVERALL" && view?.hasData === false;
                ...
                <div className="relative">
                  ...
                  <ListboxOptions className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-vdcWhite dark:bg-vdcGrey shadow-lg ring-1 ring-black/5 focus:outline-none">
                    {pillKeys.map((key) => {
                      const isDisabled =
                        key !== "OVERALL" &&
                        viewByKey.get(key)?.hasData === false;
                      return (
                        <ListboxOption
                          key={key}
                          value={key}
...
function headerTitle(view: SeasonView): string {
  return view.key === "OVERALL" ? "All Tiers" : `${view.key} Tier`;
}
