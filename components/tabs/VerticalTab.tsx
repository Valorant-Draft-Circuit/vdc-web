...
export default function VerticalTab(props: { tabElements: FranchiseTeams[] }) {
  const searchParams = useSearchParams();
  const queryParam = searchParams.get("team")?.toLowerCase();
    ...
    <>
      <TabGroup
        ...
        onChange={(index) => {
          ...
          const newUrl = `${pathname}?team=${newTier}`;
          router.push(newUrl);
        }}
          ...
          <div className="flex flex-row gap-2 sticky top-26 self-start">
            <div className="p-4 drop-shadow-lg bg-gray-100 dark:bg-vdcGrey rounded-xl w-full">
              <TabList className="flex flex-row items-start gap-5 rounded-2xl drop-shadow-2xl">
                {props.tabElements.map(({ name, color }) => (
                  <Tab
                    key={name}
