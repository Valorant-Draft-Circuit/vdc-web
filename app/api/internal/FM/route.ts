...
export async function GET() {
  ...
  if (!hasAccess(userRoles, [Roles.ADMIN, Roles.LEAD_TECH, Roles.LEAGUE_AGM, Roles.LEAGUE_GM])) {
    return NextResponse.json({
  ...
  const formattedContracts = contracts.map((contract) => {
    ...
    const formattedContract = {
      ...
      contractStatus: contract.Status?.contractStatus as string | ContractStatus | null | undefined,
      contractRemaining: contract.Status?.contractRemaining,
      mmr: contract.PrimaryRiotAccount?.MMR?.mmrEffective,
      tier:
        (contract.PrimaryRiotAccount?.MMR?.mmrEffective ?? -1) < tierlines.RECRUIT.max
          ? 'RECRUIT'
          : (contract.PrimaryRiotAccount?.MMR?.mmrEffective ?? -1) < tierlines.PROSPECT.max
          ? "PROSPECT"
          : (contract.PrimaryRiotAccount?.MMR?.mmrEffective ?? -1) < tierlines.APPRENTICE.max
          ? 'APPRENTICE'
          : (contract.PrimaryRiotAccount?.MMR?.mmrEffective ?? -1) < tierlines.EXPERT.max
          ? 'EXPERT'
          : (contract.PrimaryRiotAccount?.MMR?.mmrEffective ?? -1) < tierlines.MYTHIC.max
          ? 'MYTHIC'
          : 'N/A',
